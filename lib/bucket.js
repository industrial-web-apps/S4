...
_.extend(Bucket.prototype, {
    ...
    insertFile: function (key, fileStream, finishedCb) {
        ...
        var fileInsert = function (cb) {
            self._db.run('INSERT INTO files VALUES ($key, $fileId)', { $key: key, $fileId: Date.now() }, function (err) {
                if (err) {
                    if (err && err.toString() === 'Error: SQLITE_CONSTRAINT: UNIQUE constraint failed: files.fileId')
                        err = new Error('Fatal Error: fileID temporary date already exists');
                    else if (err && err.toString() === 'Error: SQLITE_CONSTRAINT: UNIQUE constraint failed: files.key')
                        err = new Error('Key already exists');
                }
                rowId = this.lastID;
                cb(err);
            });
        ...
        var updateStorage = function (cb) {
            storage.update(fileId, function (err, header) {
                if (err)
                    return cb(err);
        ...
        async.parallel([generateHash, saveFile], function (err) {
            ...
            async.parallel([updateStorage, updateFileId], function (err) {
                finishedCb && finishedCb(err, { rowId: rowId, key: key, fileId: fileId });
            });
        });
